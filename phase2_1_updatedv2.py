...
def merge_short_segments(segments: list, min_duration_s: float = 15.0) -> list:
    """
    Repeatedly absorb segments shorter than min_duration_s into their
    longest neighbour until all segments meet the minimum.
    No segments are ever dropped — time is always conserved.
    """
    if not segments:
        return segments
    ...
    while changed:
        ...
        # Find the shortest segment that is below the threshold
        short_idx = None
        shortest  = float('inf')
        ...
        if short_idx is None:
            break   # all segments already meet the minimum

        # Decide which neighbour to absorb into
        ...
        # Merge: extend the target segment to cover the short one's time
        ...
        # consistency stays as the target's — it's the dominant segment
